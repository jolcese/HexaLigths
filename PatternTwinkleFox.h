...
uint8_t gTwinkleFoxSpeed = 4;

// Overall twinkle density.
// 0 (NONE lit) to 8 (ALL lit at once).
// Default is 5.
uint8_t gTwinkleFoxDensity = 5;
...
// If gTwinkleFoxAutoBackground is set to 1,
// then for any palette where the first two entries
// are the same, a dimmed version of that color will
// automatically be used as the background color.
boolean gTwinkleFoxAutoBackground = 0;
...
//  should light at all during this cycle, based on the gTwinkleFoxDensity.
CRGB computeOneTwinkle( uint32_t ms, uint8_t salt)
{
  uint16_t ticks = ms >> (8-gTwinkleFoxSpeed);
  ...
  uint8_t bright = 0;
  if( ((slowcycle8 & 0x0E)/2) < gTwinkleFoxDensity) {
...
void fTwinkleFox()
{
  ...
  // if gTwinkleFoxAutoBackground == 1, and the first two colors of
...
String getTwinkleFoxAutoBackground() {
  return String(gTwinkleFoxAutoBackground);
}
